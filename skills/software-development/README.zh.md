...
| 模式 | 何时 | 跳过的 gate |
|------|------|-------------|
| Lite | 低风险、单点小改、不动架构 | Gate 1、1R、2、4、5、6 |
...
| 1 Feature Design | `woos-feature-design` | `docs/engineering/<version>/<feature-id>-design.md`。涉及时引用 `api-design` / `database-migrations` / `deployment-patterns`。偏离基线用 `architecture-decision-records` 记录。 |
| 1R Design Review | `woos-design-review-gate` | Fresh context 里独立架构师评审；`PASS` / `REQUEST_CHANGES`。连续 2 轮失败 → `woos-human-handoff`。 |
| 2 Story Decomposition | `woos-story-decomposition`（+ `woos-product-planner` 评审） | 单个 per-feature `docs/stories/<version>/<feature-id>/plan.md`：`ID \| AC \| Depends \| Diff Scope`。不再有每个 story 的 narrative 文档。 |
| 3 Story Loop | `tdd-workflow`、`coding-standards`、`verification-loop` | 按 DAG 顺序逐 story：RED→GREEN→REFACTOR → 实现 → 验证。条件触发：`database-migrations`、`e2e-testing`、`browser-qa`。被 block 的 story 不阻塞独立 story。 |
| 4 Executable Acceptance | `woos-executable-acceptance-gate` | 所有 PRD AC 映射到可执行检查。缺自动化 = blocker。 |
| 5 Deviation Control | `woos-deviation-control-gate` | 实现 vs PRD/架构/设计；未解决的偏离阻塞推进。有意偏离需要 ADR。 |
| 6 Requirement Traceability | （内置） | `docs/traceability/<version>/<feature-id>-traceability.md`：PRD AC → design → code → test → 状态表。零 ❌ 才能过。 |
...
| Post Workflow Memory | `woos-workflow-memory` | 持久化失败/返工模式与 story 拆分质量信号，供下次运行参考。 |

## 故事计划（Gate 2）

Gate 2 输出单个 per-feature 文件：`docs/stories/<version>/<feature-id>/plan.md`。一张 4 列表就是全部产物：
...
为什么这么薄：PRD AC 就是规约，diff scope 里的测试就是验证，`git restore -- <diff_scope>` 就是回滚。Story 的状态与失败日志属于运行时状态，存在 `run-manifest.yaml` 里。Sizing 规则：1 story 覆盖 1 个 PRD AC（强耦合且共享 test setup 时硬上限 3 条）；不存在依赖关系的两个 story 不能共享文件。权威 schema 见 `woos-story-decomposition/SKILL.md`。
