...
| 2 Story Loop | `tdd-workflow`、`coding-standards`、`verification-loop` | 按 Plan 内 Story Table 的 DAG 顺序逐 story：RED→GREEN→REFACTOR → 实现 → 验证。条件触发：`database-migrations`、`e2e-testing`、`browser-qa`。被 block 的 story 不阻塞独立 story。 |
| 3 Review | `woos-code-review-gate` → `woos-code-reviewer`（命中触发条件时加 `woos-security-reviewer`、适用时加 `woos-production-audit`） | Fresh context + 知识注入（E1）。一次过的硬性检查：代码 / 安全 findings（结构化表 E2）、**AC 覆盖**（每条 PRD AC 都有通过的测试）、**范围漂移**（diff 内每个文件都在某个 story 的 Diff Scope 里）、baseline / deviation 对齐。 |
| 4 Ship | `woos-pr-readiness` + `git-workflow` | 再跑一次 `verification-loop` 作为兜底，机械地从 Plan 的 Story Table + 最后一次测试结果生成 `docs/traceability/<version>/<feature-id>-traceability.md`，附到 PR body，然后 `gh pr create`。 |
